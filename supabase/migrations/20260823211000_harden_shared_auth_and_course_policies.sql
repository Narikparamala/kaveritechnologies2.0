-- ============================================================================
-- KAVERI PLATFORM — SHARED AUTHORIZATION HARDENING
-- ============================================================================
-- IMPORTANT: Review/test before applying to production.
--
-- Purpose:
--   * remove temporary preview ALL policies from shared authorization tables;
--   * make profiles.role / profiles.is_active trustworthy authorization fields;
--   * preserve normal signed-in profile editing;
--   * preserve Super Admin role management;
--   * prevent students/faculty from changing their authorization fields;
--   * prevent arbitrary course_faculty assignment changes;
--   * preserve public published-course reads and legitimate faculty/admin flows.
-- ============================================================================

begin;

-- --------------------------------------------------------------------------
-- 1. Canonical active-role helper functions
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- 2. Profiles: protect authorization fields at the database boundary
-- --------------------------------------------------------------------------
...
begin
  ...
  -- Email identity belongs to Supabase Auth / trusted admin workflows.
  -- Role and active status are authorization fields. Only an active
  -- Super Admin may alter them through the browser/API.
...
-- The LMS has legitimate authenticated cross-profile reads (faculty/student
-- lists, submissions, leaderboards, admin pages). Keep those reads while
...
-- Signed-in users may edit their own row, but the trigger above blocks role,
-- is_active, email and id changes for non-admin users.
...
-- Active Super Admin can manage any profile, including role/is_active.
...
-- No public/anonymous profile mutations. Keep authenticated table privileges
-- because RLS + the authorization-field trigger enforce row/field rules.
revoke all on table public.profiles from anon;
grant select, update on table public.profiles to authenticated;

-- --------------------------------------------------------------------------
-- 3. Courses: remove preview ALL access while preserving catalog reads
-- --------------------------------------------------------------------------
...
-- Public website may read published courses.
create policy courses_public_select
on public.courses for select
to anon
using (is_published = true);

-- Signed-in users can read the course catalog. Authorization-sensitive writes
...
-- Direct creation is Super Admin only. Faculty course creation continues via
-- the existing validated create_faculty_course(jsonb) SECURITY DEFINER RPC.
create policy courses_insert_hardened
on public.courses for insert
to authenticated
with check ((select public.is_admin()));

-- Active Super Admin may update any course; assigned active faculty may update
-- their own assigned course.
...
-- --------------------------------------------------------------------------
-- 4. course_faculty: assignment rows are authorization data
-- --------------------------------------------------------------------------
...
-- Faculty dashboards and admin management need to read assignments.
create policy course_faculty_select_authenticated
on public.course_faculty for select
to authenticated
using (true);

-- Only an active Super Admin may directly assign/remove faculty. The existing
-- create_faculty_course RPC can still create the creator's assignment inside
-- its validated SECURITY DEFINER transaction.
...
-- --------------------------------------------------------------------------
-- 5. Explicit grants for helpers used by RLS / application
-- --------------------------------------------------------------------------
