...
import {
  LayoutDashboard, BookOpen, BookPlus, FileText, ClipboardList,
  MessageSquare, HelpCircle, FolderKanban, Megaphone, BarChart2,
    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className={cn('fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 flex flex-col transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:z-auto', sidebarOpen ? 'translate-x-0' : '-translate-x-full')}>
          ...
          <DemoSidebar navItems={NAV_ITEMS} basePath="/demo/faculty" identity={{ name: DEMO_FACULTY.full_name, email: DEMO_FACULTY.email, roleLabel: 'Faculty / Trainer · Demo' }} />
        </aside>
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          <div className="lg:hidden flex items-center gap-3 px-4 py-3 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 flex-shrink-0">
            ...
            <span className="font-semibold text-slate-900 dark:text-white">Faculty Demo</span>
...
export default function DemoFacultyLayout() {
  return (
    <DemoProvider role="faculty">
      <Inner />
    </DemoProvider>
  );
}
