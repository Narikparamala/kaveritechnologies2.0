...
import {
  Users, ChevronLeft, Loader2, CheckCircle, XCircle, AlertCircle, UserCheck,
  Download
} from 'lucide-react';
...
import {
  getFacultySessions, getSessionAttendance, markAttendance,
...
export default function FacultySessionAttendancePage() {
  ...
  const handleExportCSV = () => {
    const rows = [
      ['Name', 'Email', 'Status', 'Joined At', 'Marked By'].join(','),
      ...attendance.map(a => [
    ...
    a.href = url;
    a.download = `attendance-${session?.title?.replace(/\s+/g, '-')}-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        ...
        <div className="card p-4 text-center">
          <p className="text-2xl font-bold text-amber-600">{stats.excused}</p>
          <p className="text-xs text-slate-500">Excused</p>
        </div>
        <div className="card p-4 text-center">
          <p className="text-2xl font-bold text-slate-600">{stats.registered}</p>
      ...
      {attendance.length === 0 ? (
        <EmptyState
          ...
          description="No students have registered for this session yet."
        />
      ) : (
        <div className="card overflow-hidden">
          <table className="w-full">
            <thead className="bg-slate-50 dark:bg-slate-800">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400">Student</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400">Status</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400">Joined</th>
              ...
              {attendance.map(a => (
                <tr key={a.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  ...
                  <td className="px-4 py-3 text-xs text-slate-500">
                    {a.joined_at ? new Date(a.joined_at).toLocaleTimeString() : '-'}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button
                        onClick={() => handleMarkAttendance(a.student_id, 'attended')}
                        disabled={saving}
                      ...
                      <button
                        onClick={() => handleMarkAttendance(a.student_id, 'absent')}
                        disabled={saving}
                      ...
                      <button
                        onClick={() => handleMarkAttendance(a.student_id, 'excused')}
                        disabled={saving}
